...
class Tree(Plant):
	...
	"""
	def __init__(self, name: str, height_cm: int, age_days: int, trunk_diameter: int):
		super().__init__(name, height_cm, age_days)
...
class Vegetable(Plant):
	...
	def get_info(self):
		"""
		Displays the vegetable's information including its harvest season and nutritional value.
		"""
		extra_info = f"{self.harvest_season} harvest"
		super().get_info("Vegetable", extra_info)
		print(f"${self.name.capitalize()} is rich in {self.nutritional_value}")
  
def ft_plant_types():
	"""
	Function to create multiple Plant instances of different types and display their information.
	"""
	flower = Flower("rose", 30, 60, "red")
	tree = Tree("oak", 300, 3650, 120)
	vegetable = Vegetable("carrot", 20, 90, "Spring", "Vitamin A")

	print("=== Plant Types Output ===")
	flower.get_info()
	flower.bloom()
	print("\n")
	tree.get_info()
